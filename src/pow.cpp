...
{
    /* current difficulty formula, dutcoin - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
...
unsigned int GetLegacyNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock, bool fProofOfStake)
{
    ...
    // dutcoin coin: target change every block
    // dutcoin coin: retarget with exponential moving toward target spacing
    uint256 bnNew;
    bnNew.SetCompact(pindexPrev->nBits);
