...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::dutcoin);
    ...
    if (!settings.contains("nAnonymizedutcoinAmount"))
        settings.setValue("nAnonymizedutcoinAmount", 1000);

    nAnonymizedutcoinAmount = settings.value("nAnonymizedutcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizedutcoinAmount"))
        SoftSetArg("-anonymizedutcoinamount", settings.value("nAnonymizedutcoinAmount").toString().toStdString());
...
//            return settings.value("fShowMasternodesTab");
        ...
        case AnonymizedutcoinAmount:
            return QVariant(nAnonymizedutcoinAmount);
        case Listen:
...
//            break;
        ...
        case AnonymizedutcoinAmount:
            nAnonymizedutcoinAmount = value.toInt();
            settings.setValue("nAnonymizedutcoinAmount", nAnonymizedutcoinAmount);
            emit anonymizedutcoinAmountChanged(nAnonymizedutcoinAmount);
            break;
        case CoinControlFeatures:
