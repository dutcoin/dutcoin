# dutcoin RPC Commands and arguments
...
The configuration file is not automatically created; you can create it using your favorite plain-text editor. By default, dutcoin-qt (or dutcoind) will look for a file named dutcoin.conf in the dutcoin data directory, but both the data directory and the configuration file path may be changed using the -datadir and -conf command-line arguments.

| Platform	   |Path to data folder           | Typical path to configuration file                                                 |
|------------|--------------------------------|------------------------------------------------------------------------------------|
| Linux	     | ~/	                            | /home/username/.dutcoin/dutcoin.conf                                               |
| macOS      | ~/Library/Application Support/	| /Users/username/Library/Application Support/Eletra/dutcoin.conf                    |
| Windows	   | %APPDATA%	                    | (Vista-10) C:\Users\username\AppData\Roaming\dutcoin\dutcoin.conf                  |
|            |                                | (2000-XP) C:\Documents and Settings\username\Application Data\dutcoin\dutcoin.conf |

Note: if running dutcoin in testnet mode, the sub-folder testnet will be appended to the data directory automatically. Command line arguments

## These commands are accurate as of dutcoin Core version 2.0.0:
...
Start dutcoin Core Daemon with option:
<table>
<tr> <td>dutcoind</td><td>[options]</td></tr>
</table>
...
<tr> <td>--conf=<file></td><td>Specify configuration file (default: dutcoin.conf)</td></tr>
...
<tr> <td>--pid=<file></td><td>Specify pid file (default: dutcoind.pid)</td></tr>
...
<tr> <td>--reindexmoneysupply</td><td>Reindex the dutcoin and zECA money supply statistics on startup</td></tr>
...
<tr> <td>--paytxfee=<amt></td><td>Fee (in dutcoin/kB) to add to transactions you send (default: 0.00)
...
<tr> <td>--minrelaytxfee=<amt></td><td>Fees (in dutcoin/Kb) smaller than this are considered zero fee for relaying (default: 0.0001)
...
<tr> <td>-litemode=<n></td><td>Disable all dutcoin specific functionality (Masternodes, Zerocoin, wiftX, Budgeting) (0-1, default: 0)
</table>
...
<tr> <td>--ecastake=<n></td><td>Enable or disable staking functionality for dutcoin inputs (0-1, default: 1)
...
This documentation lists all available RPC commands as of dutcoin version 2.0.0, and limited documentation on what each command does. For full documentation of arguments, results and examples, type help ( “command” ) to view full details at the console. You can enter commands either from Tools > Debug console in the QT wallet, or using dutcoin-cli
...
#### debug ( 0 | 1 | addrman | alert | bench | coindb | db | lock | rand | rpc | selectcoins | mempool | mempoolrej | net | proxy | prune | http | libevent | dutcoin )
...
#### preparebudget "proposal-name" "url" payment-count block-start "dutcoin-address" monthy-payment
...
#### submitbudget "proposal-name" "url" payment-count block-start "dutcoin-address" monthy-payment "fee-tx"
...
PoW Only: Return if the server is set to generate coins or not. The default is false. It is set with the command line argument -gen (or dutcoin.conf setting gen) It can also be set with the setgenerate call.
...
#### validateaddress “dutcoinaddress”

Return information about the given dutcoin address.

#### verifymessage “dutcoinaddress” “signature” “message”
...
#### bip38decrypt "dutcoinaddress" "passphrase"

Decrypts and then imports password protected private key.

#### bip38encrypt "dutcoinaddress" "passphrase"

Encrypts a private key corresponding to 'dutcoinaddress'.

#### dumpprivkey “dutcoinaddress”

Reveals the private key corresponding to ‘dutcoinaddress’. Then the importprivkey can be used with this output
...
DEPRECATED. Returns the current dutcoin address for receiving payments to this account.
...
Returns a new dutcoin address for receiving payments. If ‘account’ is specified (DEPRECATED), it is added to the address book so payments received with the address will be credited to ‘account’.

#### getrawchangeaddress

Returns a new dutcoin address, for receiving change. This is for use with raw transactions, NOT normal use.

#### getreceivedbyaccount “account” ( minconf addlockconf )

DEPRECATED. Returns the total amount received by addresses with <account> in transactions with specified minimum number of confirmations.

#### getreceivedbyaddress “dutcoinaddress” ( minconf addlockconf )

Returns the total amount received by the given dutcoinaddress in transactions with specified minimum number of confirmations.
...
#### importprivkey “dutcoinprivkey” ( “label” rescan )
...
#### sendfrom “fromaccount” “todutcoinaddress” amount ( minconf addlockconf “comment” “comment-to” )

DEPRECATED (use sendtoaddress). Sent an amount from an account to a dutcoin address.
...
#### sendtoaddress “dutcoinaddress” amount ( “comment” “comment-to” subtractfeefromamount use_is use_ps )
...
Stores the wallet decryption key in memory for ‘timeout’ seconds. This is needed prior to performing transactions related to private keys such as sending dutcoins
...
Usage:
  dutcoin-tx [options] <hex-tx> [commands]  Update hex-encoded dutcoin transaction
  dutcoin-tx [options] -create [commands]   Create hex-encoded dutcoin transaction
